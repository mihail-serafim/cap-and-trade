...
if($_SERVER['REQUEST_METHOD'] == "PUT"){
	...
	$sql = "UPDATE `user_saves` SET node_graph = '$node_graph', currency = '$currency', research = '$research', curr_emissions = '$curr_emissions', emissions_cap = '$emissions_cap'  WHERE id = $id;";
	$post_data_query = mysqli_query($conn, $sql);

	if($post_data_query){
		$json = array("status" => 1, "success" => "user nodes succesfully updated");
	}
	else{
		error_log(mysqli_error($conn));
		$json = array("status" => 0, "error" => "error updating user nodes");
	}
}
else{
